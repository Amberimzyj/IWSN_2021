...
def save_figure3():
    ...
    metric_dict = {'RTSN_3000': 'Data Size:3000 ', 'RTSN_4500': 'Data Size:4500', 'RTSN_6000': 'Data Size:6000 '}
    # metirc_order = {'X2': 1, 'MI': 2, 'Cond': 3, 'Select': 4}
    color_dict = {'RTSN_3000': 'forestgreen','RTSN_4500': 'royalblue', 'RTSN_6000': 'tomato'}
...
def save_figure4():
    ...
    Q = ax.twinx()
    Q.set_ylim(0,8)
    Q.set_ylabel('Queue index of TSN gateway')
    ...
    for csv_path in data_path.glob('*.csv'):
        ...
        inte_delay = all_data[:, 4]
        data_name = csv_path.stem
        line,  = ax.plot(res, t_5G, style_dict[data_name], color=color_dict[data_name], linewidth=0.8)
        line.set_label(f'{metric_dict[data_name]}')
        # line,  = plt.plot(res, q_t, style_dict[data_name], color=color_dict[data_name], linewidth=0.8)
        # line.set_label(f'{metric_dict[data_name]}')
        # axins.plot(res, t_5G,  style_dict[data_name], color=color_dict[data_name])
        Q.plot(res, q_t,  style_dict[data_name], color=color_dict[data_name])
    ...
    plt.legend(bbox_to_anchor=(0, 1), handles=handles, labels=labels, loc='upper left', fontsize = 'medium')
    plt.xlabel('Reserved RB numbers')
    plt.ylabel('Latency of 5G/TSN network')
    plt.show()
