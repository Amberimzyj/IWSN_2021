...
class Bandit:
    ...
    def reset(self):
        ...
        # self.inte_delay_true = np.random.randn(self.k) + classical_true_value
...
def figure_inte_delay(runs=100, time = 1000):
    bandits = []
    bandits.append(Bandit(gradient=True, k_arm = 15, step_size=0.1, gradient_baseline=True, true_reward=0))
    ...
    plt.show()
    np.savetxt('data/Bandit_data/data_risk.csv', rewards)
    # np.savetxt('data/Bandit_data/data_without_risk.csv', rewards)
